...
export async function ensureDefaultBranch(tenantId: string) {
  ...
  const settings = await query("SELECT name, country FROM tenant_settings WHERE tenant_id = $1", [tenantId]);
  const name = (settings.rows[0]?.name as string) || "Main branch";
  const country = (settings.rows[0]?.country as string) || null;
  const result = await query(
    `INSERT INTO branches (tenant_id, name, country, is_default, is_active) VALUES ($1,$2,$3,true,true) RETURNING id`,
    [tenantId, name, country],
  );
...
export async function createBranch(tenantId: string, data: Record<string, unknown>) {
  ...
  let country = (data.country as string) || null;
  if (!country) {
    const settings = await query("SELECT country FROM tenant_settings WHERE tenant_id = $1", [tenantId]);
    country = (settings.rows[0]?.country as string) || null;
  }
  const result = await query(
    `INSERT INTO branches (tenant_id, name, address, phone, country, is_default, is_active)
     VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
    [tenantId, data.name, data.address || null, data.phone || null, country, data.isDefault ?? false, data.isActive ?? true],
  );
...
export async function updateBranch(tenantId: string, id: string, updates: Record<string, unknown>) {
  ...
  const map: Record<string, string> = {
    name: "name", address: "address", phone: "phone", country: "country",
